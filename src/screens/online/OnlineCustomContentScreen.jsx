...
export default function OnlineCustomContentScreen() {
  const { gameState, setStatus, addCustomCard, removeCustomCard } = useOnlineGame()
  ...
  function submit(e) {
    e.preventDefault()
    if (!text.trim()) return
    addCustomCard({ cardType, level, text, timerSeconds })
    setText('')
  }
  ...
  return (
    <div className="screen">
      <div className="top-bar">
        <button className="back-btn" onClick={() => setStatus('lobby')}>
          ‹ Volver
        </button>
      </div>
      ...
      <div>
        ...
        <h2 className="title" style={{ fontSize: 24 }}>
          Agregá preguntas y retos
        </h2>
        <p className="subtitle" style={{ marginTop: 6 }}>
          Se van a mezclar con las del banco para todos los que estén en esta sala.
        </p>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1, overflowY: 'auto' }}>
        {gameState.customCards.length === 0 && (
          <p className="subtitle" style={{ textAlign: 'center', marginTop: 12 }}>
            Todavía no agregaron contenido propio.
          </p>
        )}
        {gameState.customCards.map((c) => (
          <div
            key={c.id}
            className="card"
      ...
      <button className="btn btn-secondary btn-block" onClick={() => setStatus('lobby')}>
        Listo
      </button>
    </div>
  )
